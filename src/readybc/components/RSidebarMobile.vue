<template>
  <EcFlex class="lg:hidden">
    ...
    <EcBox class="fixed top-0 left-0 z-40 w-screen h-screen" :class="showMenu ? '' : 'pointer-events-none'">
      ...
      >
        ...
        <EcBox class="pr-3 overflow-auto">
          <CSidebarMenu menuDirection="rtl" :unreadNotification="unreadNotification" @click-menu-item="hideMenu" />
        </EcBox>
      </EcFlex>
    </EcBox>
    <!-- Hamburger Button -->
    <EcFlex
      ...
      @click="toggleMenu"
    >
      <EcBox v-if="!showMenu" width="34" height="34" icon="Menu" />
      <EcBox v-else v-scroll-lock="true" width="34" height="34" icon="X" />
    </EcFlex>
  </EcFlex>
</template>
