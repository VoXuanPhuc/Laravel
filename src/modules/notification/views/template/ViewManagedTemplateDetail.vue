    ...
    <EcFlex class="items-center flex-wrap">
      <EcFlex class="items-center justify-between w-full flex-wrap lg:w-auto lg:mr-4">
        <EcHeadline class="text-cBlack mr-4 mb-3 lg:mb-0">
          {{ $t("notification.template.updateTemplate") }}
        </EcHeadline>
      </EcFlex>
    </EcFlex>
...
export default {
  ...
  mounted() {
    ...
    this.fetchManagedTemplateDetail()
  },
  setup() {
    const globalStore = useGlobalStore()
    // Pre-loaded
    const { managedTemplate, v$, getManagedTemplateDetail, updateManagedTemplate } = useManagedTemplateDetail()
  ...
  methods: {
    ...
    async handleClickConfirm() {
      ...
      const response = await this.updateManagedTemplate(this.managedTemplate, this.uid)
     ...
     */
    async fetchManagedTemplateDetail() {
      this.isLoading = true
      const res = await this.getManagedTemplateDetail(this.uid)
      if (res) {
        this.managedTemplate = res
      } else {
        goto("ViewManagedTemplateList")
      }
