    ...
    <EcFlex class="items-center flex-wrap">
      <EcFlex class="items-center justify-between w-full flex-wrap lg:w-auto lg:mr-4">
        <EcHeadline class="text-cBlack mr-4 mb-3 lg:mb-0">
          {{ $t("dependencyScenario.title.newDependency") }}
        </EcHeadline>
      </EcFlex>
    </EcFlex>
    ...
    <EcBox class="width-full mt-8 px-4 sm:px-10">
      ...
      <EcFlex v-if="!isLoading" class="mt-6">
        ...
        <EcButton variant="primary" class="ml-4" @click="handleClickCreate">
          {{ $t("dependencyScenario.buttons.create") }}
        </EcButton>
      </EcFlex>
...
import { useDependencyScenarioNew } from "@/modules/dependency/use/dependency/useDependencyScenarioNew"
...
export default {
  ...
  setup() {
    ...
    // Pre-loaded
    const { dependencyScenario, valdator$, createNewDependencyScenario } = useDependencyScenarioNew()
  ...
  methods: {
    ...
    async handleClickCreate() {
      ...
      const response = await this.createNewDependencyScenario(this.dependencyScenario)
